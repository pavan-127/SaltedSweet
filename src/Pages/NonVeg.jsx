...
function NonVeg() {
    ...
    const filteredItems = nonVegItems.filter(item => {
        ...
        if (filter === "100to200") return item.price >= 100 && item.price <= 200;
        if (filter === "200to300") return item.price > 200 && item.price <= 300;
        if (filter === "above300") return item.price > 300;
        return true;
    });
    ...
    return (
        <>
            <section className="container py-5">
                <h1 className="text-center text-danger mt-5 mb-4">Non-Veg Items</h1>
                <div className="row">
                    {/* Filter Button */}
                    ...
                    {showFilter && (
                        <div className="col-md-3">
                            ...
                            <div className="list-group">
                                ...
                                <button className={`list-group-item list-group-item-action ${filter === "below100" ? "active" : ""}`} onClick={() => setFilter("below100")}>Below ₹100</button>
                                <button className={`list-group-item list-group-item-action ${filter === "100to200" ? "active" : ""}`} onClick={() => setFilter("100to200")}>₹100 - ₹200</button>
                                <button className={`list-group-item list-group-item-action ${filter === "200to300" ? "active" : ""}`} onClick={() => setFilter("200to300")}>₹200 - ₹300</button>
                                <button className={`list-group-item list-group-item-action ${filter === "above300" ? "active" : ""}`} onClick={() => setFilter("above300")}>Above ₹300</button>
                            </div>
                        </div>
                    )}
                    ...
                    <div className={`col-md-${showFilter ? '9' : '12'}` }>
                                ...
                                <div className="col-md-4" key={index}>
                                    <div className="card nonveg-card">
                                        ...
                                        <div className="card-body text-center">
                                            ...
                                            <button className="btn btn-danger w-100" onClick={() => dispatch(addToCart(item))}>Add to Cart</button>
                                        </div>
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className="d-flex justify-content-center mt-4">
                            ...
                            {[...Array(totalPages)].map((_, index) => (
                                <button 
                                    key={index} 
                                    className={`btn ${currentPage === index + 1 ? 'btn-danger' : 'btn-light'} mx-1`} 
